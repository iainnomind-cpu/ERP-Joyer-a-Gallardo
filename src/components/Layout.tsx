...
import { Users, Package, ShoppingCart, Settings, LayoutDashboard, Trello, Megaphone, FileText, LogOut, User, Globe, MessageSquare } from 'lucide-react';
...
type LayoutProps = {
  children: ReactNode;
  currentModule: 'dashboard' | 'crm' | 'inventory' | 'sales' | 'config' | 'kanban' | 'marketing' | 'quotes' | 'ecommerce' | 'inbox';
  onModuleChange: (module: 'dashboard' | 'crm' | 'inventory' | 'sales' | 'config' | 'kanban' | 'marketing' | 'quotes' | 'ecommerce' | 'inbox') => void;
...
export default function Layout({ children, currentModule, onModuleChange, currentUser, onLogout, permissions }: LayoutProps) {
  const allModules = [
    ...
    { id: 'inbox' as const, name: 'Inbox', icon: MessageSquare },
